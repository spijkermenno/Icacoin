...
def main():
    ...
    tmpdir = "%s/icacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and icacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "icacoind"]) is not None:
            print("%sWARNING!%s There is already a icacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "LITECOIND" not in os.environ:
        os.environ["LITECOIND"] = build_dir + '/src/icacoind' + exeext
        os.environ["LITECOINCLI"] = build_dir + '/src/icacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `icacoin-cli help` (`rpc_interface.txt`).
