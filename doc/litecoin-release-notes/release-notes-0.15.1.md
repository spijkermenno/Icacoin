Icacoin Core version *0.15.1* is now available from:

  <https://download.icacoin.org/icacoin-0.15.1/>
...
A number of changes to the way Icacoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hans per kB (0.00001 ICA). This is to prepare for dropping the minimum transaction fee to 0.1 hans per kB (0.0001 ICA) in the next release.
